...
const DashboardCards: React.FC<Props> = ({user, refetch}) => {
    const cards = user.cards.map((card: Card) => card.status !== "REJECTED" && card.status !== "WAITING_APPROVAL");
    ...
    return (
        <React.Fragment key={user.id}>
            {cards.length === 0
                ...
                :   <React.Fragment key={user.id}>
                        {user.cards.map((card: Card, index: number) => card.status !== "REJECTED" && card.status !== "WAITING_APPROVAL" && (
